...
from backend.models import Produk, RoleEnum
from backend.core.deps import get_current_user, require_kasir, require_admin
...
def get_inventory_by_id(
    ...
    item = db.query(Produk).filter(Produk.id == item_id,
                                   Produk.is_active == True).first()
    if not item:
