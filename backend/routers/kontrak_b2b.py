...
def _build_b2b_pdf(kontrak: KontrakB2B, pembuat_username: str) -> bytes:
    ...
    def draw_paragraph(text: str, font="Helvetica", size=10, indent=0,
                        gap_after=4, color=(0, 0, 0), line_spacing=14):
        c.setFont(font, size)
    ...
    draw_paragraph(f"1.  Nama Perusahaan  : {p1_nama}", font="Helvetica-Bold", size=10, gap_after=2)
    ...
    draw_paragraph(
        ...
        size=10, indent=7*mm, gap_after=10
    )

    # ── PIHAK KEDUA ───────────────────────────────────────────
    ensure(80)
    draw_paragraph(f"2.  Nama Perusahaan  : {p2_nama}", font="Helvetica-Bold", size=10, gap_after=2)
    ...
    c.drawString(ML, y[0], "Dokumen ini ditandatangani secara elektronik dan dilindungi dengan kriptografi SHA-256.")
    y[0] -= 11
