    ...
    try:
        ...
        for trx in transaksis:
            kontrak = db.query(Kontrak).filter(
                Kontrak.transaksi_id == trx.id).first()
            if kontrak:
                pdf_path = os.path.join(os.path.dirname(
                    __file__), "data", "contracts", f"{kontrak.kode}.pdf")
                if not os.path.exists(pdf_path):
                    print(
                        f"[*] PDF hilang untuk {kontrak.kode} (Trx {trx.kode}). Menghapus kontrak lama dan meregenerasi...")
                    db.delete(kontrak)
                    db.commit()
                    # Generate ulang
