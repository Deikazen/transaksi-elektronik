from backend.models import Transaksi, ItemTransaksi, Kontrak, IOLog
...
def migrate():
    ...
    try:
        ...
        for tx in txs:
            ...
            exists = db.query(IOLog).filter(
                IOLog.transaction_kode == tx.kode).first()
            if exists:
                print(f"Logs for {tx.kode} already exist, skipping.")
                continue

            items = db.query(ItemTransaksi).filter(
                ItemTransaksi.transaksi_id == tx.id).all()
            kontrak = db.query(Kontrak).filter(
                Kontrak.transaksi_id == tx.id).first()
